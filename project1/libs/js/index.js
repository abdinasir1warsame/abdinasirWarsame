...
var geoWorldMap = L.tileLayer(
  'https://maps.geoapify.com/v1/tile/osm-bright/{z}/{x}/{y}.png?&apiKey=63630b23ded941c08ad75ae54eb01087',
  {
    attribution:
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        window[noSpaceName] = new Country(
          name,
          iso_a2,
          iso_a3,
          iso_n3,
...
function getWeatherInfo(country) {
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.status.name == 'ok') {
        country.weather_current.push(
          result['data']['current']['temp'] - 273.15,
          result['data']['current']['feels_like'] - 273.15,
          result['data']['current']['weather']['0']['main'],
          result['data']['current']['weather']['0']['icon']
        );

        country.weather_tomorrow.push(
          result['data']['daily']['0']['temp']['day'] - 273.15,
          result['data']['daily']['0']['feels_like']['day'] - 273.15,
          result['data']['daily']['0']['weather']['0']['main'],
          result['data']['daily']['0']['weather']['0']['icon']
        );

        country.weather_2days.push(
          result['data']['daily']['1']['temp']['day'] - 273.15,
          result['data']['daily']['1']['feels_like']['day'] - 273.15,
          result['data']['daily']['1']['weather']['0']['main'],
          result['data']['daily']['1']['weather']['0']['icon']
        );
      }
    },
    error: function (jqXHR, textStatus, errorThrown) {
...
function getRESTCountryInfo(country) {
  $.ajax({
    ...
    success: function (result) {
      if (result.status.name == 'ok') {
        // Assuming the first language is the primary one
        var firstLanguageKey = Object.keys(result['data'][0]['languages'])[0];
        country.languages = result['data'][0]['languages'][firstLanguageKey];

        // Extracting currency information
        var firstCurrencyKey = Object.keys(result['data'][0]['currencies'])[0];
        var currencyInfo = result['data'][0]['currencies'][firstCurrencyKey];
        country.currencyCode = firstCurrencyKey;
        country.currencyName = currencyInfo['name'];
        country.currencySymbol = currencyInfo['symbol'];

        country.topLevelDomain = result['data'][0]['tld'][0];
        country.callingCode =
          result['data'][0]['idd']['root'] +
          result['data'][0]['idd']['suffixes'][0];
      }
    },
    error: function (jqXHR, textStatus, errorThrown) {
      console.log(JSON.stringify(jqXHR));
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.status.name == 'ok') {
        ...
        country.news_article1.push(
          result['data']['articles']['0']['title'],
          result['data']['articles']['0']['url'],
          result['data']['articles']['0']['source']['Name'],
          result['data']['articles']['0']['publishedAt']
        );

        country.news_article2.push(
          result['data']['articles']['1']['title'],
          result['data']['articles']['1']['url'],
          result['data']['articles']['1']['source']['Name'],
          result['data']['articles']['1']['publishedAt']
        );

        country.news_article3.push(
          result['data']['articles']['2']['title'],
          result['data']['articles']['2']['url'],
          result['data']['articles']['2']['source']['Name'],
          result['data']['articles']['2']['publishedAt']
        );
      }
    },
    error: function (jqXHR, textStatus, errorThrown) {
  ...
  states: [
    {
      ...
      onClick: function (btn, map) {
        ...
        document.getElementById(
          'Modal1Title'
        ).innerHTML = `${currentCountry.name} Information`;
        ...
        document.getElementById('countryInfoPopulation').innerHTML = `${(
          currentCountry.population / 1000000
        ).toFixed(1)} M`;
  ...
  states: [
    {
      ...
      onClick: function (btn, map) {
        ...
        document.getElementById(
          'Modal2Title'
        ).innerHTML = `Currency for ${currentCountry.name}`;
        document.getElementById(
          'currencyInfoCurrencyCode'
        ).innerHTML = `${currentCountry.currencyCode} (${currentCountry.currencySymbol})`;
        document.getElementById('currencyInfoCurrencyName').innerHTML =
          currentCountry.currencyName;
        document.getElementById(
          'currencyInfoExchange'
        ).innerHTML = `${currentCountry.exchangeRate}`;
        document.getElementById('currencyInfoTLD').innerHTML =
          currentCountry.topLevelDomain;
        document.getElementById(
          'currencyInfoCalling'
        ).innerHTML = `+${currentCountry.callingCode}`;
      },
    },
    {
      icon: '&#x238C;',
  ...
  states: [
    {
      ...
      onClick: function (btn, map) {
        ...
        (twoDays = days[twoDays]), $('#weatherModal').modal('show');
        ...
        document.getElementById(
          'Modal3Title'
        ).innerHTML = `3 Day Weather Forecast for ${currentCountry.name}`;
        document.getElementById(
          'currentIcon'
        ).src = `http://openweathermap.org/img/w/${currentCountry.weather_current[3]}.png`;
        document.getElementById('currentTemp').innerHTML = `${Math.floor(
          currentCountry.weather_current[0]
        )}°C`;
        document.getElementById('currentFeelsLike').innerHTML = `${Math.floor(
          currentCountry.weather_current[1]
        )}°C`;
        document.getElementById('currentConditions').innerHTML =
          currentCountry.weather_current[2];
        document.getElementById(
          'tomorrowIcon'
        ).src = `http://openweathermap.org/img/w/${currentCountry.weather_tomorrow[3]}.png`;
        document.getElementById('tomorrowTemp').innerHTML = `${Math.floor(
          currentCountry.weather_tomorrow[0]
        )}°C`;
        document.getElementById('tomorrowFeelsLike').innerHTML = `${Math.floor(
          currentCountry.weather_tomorrow[1]
        )}°C`;
        document.getElementById('tomorrowConditions').innerHTML =
          currentCountry.weather_tomorrow[2];
        document.getElementById('2DayName').innerHTML = twoDays;
        document.getElementById(
          '2dayIcon'
        ).src = `http://openweathermap.org/img/w/${currentCountry.weather_2days[3]}.png`;
        document.getElementById('2dayTemp').innerHTML = `${Math.floor(
          currentCountry.weather_2days[0]
        )}°C`;
        document.getElementById('2dayFeelsLike').innerHTML = `${Math.floor(
          currentCountry.weather_2days[1]
        )}°C`;
        document.getElementById('2dayConditions').innerHTML =
          currentCountry.weather_2days[2];
      },
    },
    {
      icon: '&#x238C;',
  ...
  states: [
    {
      ...
      onClick: function (btn, map) {
        if (currentCountry.news_article1[0]) {
          ...
          document.getElementById(
            'Modal4Title'
          ).innerHTML = `Latest Top News Stories for ${currentCountry.name}`;
          document.getElementById('article1Link').href =
            currentCountry.news_article1[1];

          document.getElementById('article1Title').innerHTML =
            currentCountry.news_article1[0];
          document.getElementById(
            'article1Source'
          ).innerHTML = `<em>Source: ${currentCountry.news_article2[2]}</em>`;
          document.getElementById('article2Link').href =
            currentCountry.news_article2[1];

          document.getElementById('article2Title').innerHTML =
            currentCountry.news_article2[0];
          document.getElementById(
            'article2Source'
          ).innerHTML = `<em>Source: ${currentCountry.news_article2[2]}</em>`;
          document.getElementById('article3Link').href =
            currentCountry.news_article3[1];

          document.getElementById('article3Title').innerHTML =
            currentCountry.news_article3[0];
          document.getElementById(
            'article3Source'
          ).innerHTML = `<em>Source: ${currentCountry.news_article3[2]}</em>`;
        } else {
